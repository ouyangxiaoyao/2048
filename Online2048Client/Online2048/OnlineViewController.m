...
#define kPlaceX @"X"
#define kPlaceY @"Y"
#define scrinW   [UIScreen mainScreen].bounds.size.width
#define  scrinH [UIScreen mainScreen].bounds.size.height
...
//        _currentArray存储的是字典，字典的键是label的数字标签，值是label的位置
    ...
    NSDictionary *dic=[NSDictionary dictionaryWithObjectsAndKeys:_currentArray,@"currentArray",nil];
...
-(void)addLuckyLabel:(NSInteger)random WithNum:(NSInteger)num
{
    ...
    NSLog(@"------------------------%u  %@ %lu",random,self.emptyPlaceArray,(unsigned long)self.emptyPlaceArray.count);
...
//        NSString *dataString = [NSString stringWithFormat:@"%lu",(unsigned long)swipe.direction];
...
//        [self presentViewController:alert animated:YES completion:nil];
        DMYAlertViewController *alert = [[DMYAlertViewController alloc]init];
        alert.str = @"红方胜";
        alert.view.frame = CGRectMake((scrinW-100)/2, (scrinH-100)/2, 100, 100);
        [self presentViewController:alert animated:YES completion:nil];
        return 1;
    }
    if (redCount== 0) {
...
-(void)isGameOver
{
    ...
    if (self.isOver == YES) {
        UIAlertView *alertView =  [[UIAlertView alloc]initWithTitle:@"Game Over" message:@"Game Over" delegate:self cancelButtonTitle:@"关闭" otherButtonTitles:@"", nil];
//        alertView.tag=99;
        [alertView show];
    }
}
...
- (IBAction)onlineConnect:(UIButton *)sender {
    ...
    NSError *error = nil;
    BOOL result = [self.clientSocket connectToHost:@"192.168.43.174" onPort:2345 error:&error];
    if (result) {
        NSLog(@"链接成功");
    } else {
         NSLog(@"连接失败");
...
//        
//        NSMutableArray *dataArray = [[NSKeyedUnarchiver unarchiveObjectWithData:data] mutableCopy];
...
//        [self swipeFrom:[dataString intValue]];
//
//        self.currentArray = dataArray;
////        [self setCurrentArray];
//        [self startGameWithLastMessage];
//        self.slideView.userInteractionEnabled = YES;
//        for (LuckyLabel *label in dataArray) {
//            NSLog(@" qqqqqqqqq     %u",label.placeTag);
//        }
//        NSLog(@"接收消息成功 %@",dataArray);
